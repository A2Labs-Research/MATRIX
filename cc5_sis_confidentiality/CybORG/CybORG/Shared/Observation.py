...
class Observation:
    ...
    def add_system_info(self,
                        ...
                        exfiltration_times: int = None,
                        tunnel: bool = None,
                        suspicious_network_activity: bool = None,
                        **kwargs):
        if hostid is None:
            hostid = str(len(self.data))
        if hostid not in self.data:
        ...
        if exfiltration_times is None:
            exfiltration_times = kwargs.get("exfiltration_times", None)
        if exfiltration_times is not None:
            sys_info["exfiltration_times"] = exfiltration_times

        if tunnel is None:
            tunnel = kwargs.get("tunnel", None)
        if tunnel is not None:
            sys_info["tunnel"] = tunnel

        if suspicious_network_activity is None:
            suspicious_network_activity = kwargs.get("suspicious_network_activity", None)
        if suspicious_network_activity is not None:
            sys_info["suspicious_network_activity"] = suspicious_network_activity
    ...
    def add_suspicious_network_activity(self,
                                        hostid: str = None,
                                        suspicious_network_activity: str = None,
                                        IP_of_host_with_tunnel: str = None,
                                        **kwargs):

        self.data['Network Activity'] = [{'suspicious_network_activity': suspicious_network_activity,
                                          'IP Address': IP_of_host_with_tunnel}]
