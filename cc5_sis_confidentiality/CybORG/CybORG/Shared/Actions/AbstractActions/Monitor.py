        ...
        for child in session.children.values():
            for artifact in session.artifacts:
                if len(state.hosts[child.host].events[artifact]) > 0:
                    ...
                    if artifact == 'SuspiciousActivity':
                        obs.add_suspicious_network_activity(hostid=child.host, suspicious_network_activity=True, IP_of_host_with_tunnel=state.hosts[child.host].interfaces[1].ip_address)
                if artifact != 'SuspiciousActivity':
