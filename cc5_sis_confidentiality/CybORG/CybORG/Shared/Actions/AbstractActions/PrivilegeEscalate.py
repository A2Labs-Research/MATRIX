...
class PrivilegeEscalate(Action):
    ...
    def __init__(self, session: int, agent: str, hostname: str):
        ...
        self.tunnel_detection_prob = 0.5
    ...
    def sim_execute(self, state: State) -> Observation:
        ...
        for key, host in obs2.data.items():
            if key == 'success':
                continue
            try:
                if 'Processes' in host.keys():
                    host_processes = host['Processes']
                    for proc in host_processes:
                        if proc.get('Service Name') == 'OTService':
                            state.sessions[self.agent][target_session].can_impact_servicename = 'OTService'
                            break
                        if proc.get('Service Name') in ['AuthService', 'DBService']:
                            state.sessions[self.agent][target_session].can_impact_servicename = proc.get('Service Name')
                            state.hosts[self.hostname].unblock_traffic()
                            try:
                                if state.hosts[self.hostname].tunnel:
                                    # print(('TUNNEL EXISTS'))
                                    pass
                                else:
                                    ...
                                    rand_remote_port = 53
                                    random_local_port = randint(25000, 25555)
                                    remote_random_ip = f"10.50.{octet3}.{octet4}"
                                    tunnel_process = state.tunnel_set(self.hostname)
                                    new_session = state.add_session(host=self.hostname, agent=self.agent, user=sess.username, 
                                                                    session_type="tunnel", parent=self.session, process=tunnel_process.pid)
                                    new_connection = {"local_port": random_local_port, "Application Protocol": "tcp",
                                                    "remote_address": IPv4Address(remote_random_ip), "remote_port": rand_remote_port, 
                                                    "local_address": IPv4Address(obs2.data[self.hostname]['Interface'][1]['IP Address'])}
                                    tunnel_process.connections.append(new_connection)

                                    obs.add_session_info(hostid=self.hostname, session_id=new_session.ident, 
                                                        session_type=new_session.session_type, agent=self.agent)
                                    obs.add_process(hostid=self.hostname, pid=tunnel_process.pid, ppid=tunnel_process.ppid,
                                                    local_address=IPv4Address(obs2.data[self.hostname]['Interface'][1]['IP Address']), local_port=random_local_port, 
                                                    remote_address=IPv4Address(remote_random_ip), remote_port=rand_remote_port, 
                                                    status="open", process_type='tunnel')
                                    if random() < self.tunnel_detection_prob:
                                        state.hosts[self.hostname].events['ProcessCreation'].append({'ProcessName': 'tunnel', 'PID': tunnel_process.pid})
                                        state.hosts[self.hostname].events['NetworkConnections'].append({'local_address': IPv4Address(obs2.data[self.hostname]['Interface'][1]['IP Address']),
                                                                        'local_port': random_local_port,
                                                                        'remote_address': IPv4Address(remote_random_ip), "remote_port": rand_remote_port, 'pid': tunnel_process.pid})
