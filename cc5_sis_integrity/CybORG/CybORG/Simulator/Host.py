...
from CybORG.Shared.Scenarios.services.services_utils import move_files, create_user_pages
...
class Host(Entity):
    ...
    def __init__(self, system_info: dict, hostname: str = None, users: dict = None,
            ...
            for process in processes:
                self.processes.append(
                    Process(pid=process.get('PID'), parent_pid=process.get('PPID'), username=process.get("Username"),
                            ...
                            process_type=process.get('Process Type'), status="open"))
        ...
        self.events = {'NetworkConnections': [], 'ProcessCreation': [], 'DefaceFront': [], 'DefaceAuth': []}
        self.defaced_front_status = False
        self.defaced_auth_status = False
        self.green_complainers = []

    def change_front_deface_status(self, green_complainers):
        self.defaced_front_status = True
        self.green_complainers = green_complainers
        return self.defaced_front_status

    def change_auth_deface_status(self, green_complainers):
        self.defaced_auth_status = True
        self.green_complainers = green_complainers
        return self.defaced_auth_status
    ...
    def add_process(self, name: str, user: str, pid: int = None, ppid: int = None, path: str = None,
                    ...
                    decoy_type: DecoyType = DecoyType.NONE, connections=None, properties: Optional[List[str]] = None, status = None):
        if pid is None:
            pids = []
    ...
    def restore_services(self, source_file, destination_file):
        try:
    ...
    def restore_content(self, uuid=None):
        self.defaced_front_status = False
        self.defaced_auth_status = False
        if 'front' in self.hostname.lower():
            move_files('front', uuid=uuid)
        if 'auth' in self.hostname.lower():
            move_files('auth', uuid=uuid)
            db_initial_path = str(inspect.getfile(CybORG))[:-10] + '/Shared/Scenarios/services/' + uuid + '/initial/auth.json'
            with open(db_initial_path, "r") as json_file:
                data = json.load(json_file)
            create_user_pages(data, uuid=uuid)
