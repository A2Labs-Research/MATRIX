...
class RedMeanderSISAgent(BaseAgent):
    # a red agent that meanders through scenario 2 SIS
    def __init__(self):
    ...
    def get_action(self, observation, action_space, true_obs = None):
        ...
        if 'Op_Auth' in self.escalated_hosts:
            self.last_host = 'Op_Auth'
            return Impact(agent='Red', hostname='Op_Auth', session=session)
        elif 'Op_Database' in self.escalated_hosts:
            self.last_host = 'Op_Database'
            return Impact(agent='Red', hostname='Op_Database', session=session)
    ...
    def _process_failed_ip(self):
        ...
        if len(hosts_of_type('Op')) > 0:
            ...
            for host in hosts_of_type('Ent'):
                ...
                ip = self.host_ip_map[host]
                self.exploited_ips.remove(ip)
        elif len(hosts_of_type('User')) > 0:
            for host in hosts_of_type('User'):
                self.escalated_hosts.remove(host)
                ip = self.host_ip_map[host]
                self.exploited_ips.remove(ip)
