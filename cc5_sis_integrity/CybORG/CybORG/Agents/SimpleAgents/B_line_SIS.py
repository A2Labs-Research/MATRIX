...
class B_lineAgent_SIS(BaseAgent):
    def __init__(self):
        self.action = 0
        ...
        self.action_history = {}
        self.jumps = [0,1,2,2,2,2,5,5,5,5,9,9,9,12,12,12,12,16,17]
    ...
    def get_action(self, observation, action_space, true_obs = None):
        ...
        while True:
            if observation['success'] == True:
                self.action += 1 if self.action < 18 else 0
            else:
            ...
            # Scanning the new subnet found.
                ...
                host = [x for x in observation if 'Op_' in x][0]
                self.last_subnet = observation[host]['Interface'][0]['Subnet']
                action = DiscoverRemoteSystems(subnet=self.last_subnet, agent='Red', session=session)

            # Discover Network Services- Op_Auth
            elif self.action == 15:
                self.target_ip_address = [value for key, value in observation.items() if key != 'success'][3]['Interface'][0]['IP Address']
                # self.target_ip_address = [value for key, value in observation.items() if key != 'success'][4]['Interface'][0]['IP Address']
                action = DiscoverNetworkServices(session=session, agent='Red', ip_address=self.target_ip_address)
            ...
            # Privilege escalation on Op_Auth
            elif self.action == 17:
                action = PrivilegeEscalate(agent='Red', hostname='Op_Auth', session=session)

            # Impact on Op_Auth
            elif self.action == 18:
                action = Impact(agent='Red', session=session, hostname='Op_Auth')
