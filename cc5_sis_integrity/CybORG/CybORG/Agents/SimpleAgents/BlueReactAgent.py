...
from CybORG.Shared.Actions import Monitor, Remove, Restore, RestoreContent, CheckForCorruption
...
class BlueReactRemoveAgent(BaseAgent):
    ...
    def get_action(self, observation, action_space, true_obs = None):
        ...
        if self.last_action is not None and self.last_action == 'Monitor':
            for host_name, host_info in [(value['System info']['Hostname'], value) for key, value in observation.items() if key not in ['success', 'user_complaint']]:
...
class BlueReactRemoveRestoreContentAgent(BaseAgent):
    def __init__(self):
    ...
    def get_action(self, observation, action_space, true_obs = None):
        ...
        if self.last_action is not None and self.last_action == 'Monitor':
            for host_name, host_info in [(value['System info']['Hostname'], value) for key, value in observation.items() if key not in ['success', 'user_complaint']]:
                ...
                if 'user_complaint' in observation.keys():
                    if host_name != self.initial_host:
                        self.host_dict.pop(host_name, None)
                        target_host_name = observation['user_complaint'][0]['hostname']
                        self.host_dict[target_host_name] = "RestoreContent"
        ...
        if len(self.host_dict) == 0:
            ...
            if self.last_action == 'Remove':
                ...
                return RestoreContent(hostname=last_host, agent='Blue', session=session)
