...
from CybORG.tpr_fpr import HYPERPARAMS, generate_fp
...
class EnvironmentController:
    ...
    def __init__(self, scenario_path: str, scenario_mod: dict = None, agents: dict = None, random_topologies=False, hp=HYPERPARAMS):
    ...
    def reset(self, agent: str = None) -> Results:
        ...
        for agent_name, agent_object in self.agent_interfaces.items():
            agent_object.reset(self.state.red_init_choice)
            self.observation[agent_name] = self._filter_obs(self.get_true_state(self.INFO_DICT[agent_name]), agent_name)
        ...
        if agent is None:
            ...
            if agent=="Blue":
                self.observation[agent].combine_obs(self._filter_obs(self.execute_action(Monitor(session=0, agent='Blue')), agent_name))
    ...
    def step(self, agent: str = None, action: Action = None, skip_valid_action_check: bool = False) -> Results:
        ...
        for host, value in self.state.hosts.items():
            value.defaced_front_status = False
            value.defaced_auth_status = False
            value.events['DefaceFront'] = []
            value.events['DefaceAuth'] = []

        # Select actions and check if action is valid
        next_observation = {}
        for agent_name, agent_object in self.agent_interfaces.items():
            if agent is None or action is None or agent != agent_name:
                agent_action = agent_object.get_action(observation = self.observation[agent_name], 
                                                       action_space = agent_object.action_space.get_action_space(), 
                                                       true_obs = self.get_true_state(self.INFO_DICT[agent_name]).data)
            else:
                agent_action = action
        ...
        for agent_name, agent_object in self.agent_interfaces.items():
            ...
            reward = agent_object.determine_reward(next_observation, true_observation, self.action, self.done)
        ...
        for agent_name, agent_object in self.agent_interfaces.items():
            if agent_name == 'Blue':
                ...
                agent_observation = self._filter_obs(self.execute_action(Monitor(session=agent_session, agent='Blue')), agent_name)
                ...
                if random.random() < self.hp.get_fpr(): 
                    # Select a random src and dst to add a connection
                    data = self.observation['Blue'].data
                    src = random.choice(list(self.hostname_ip_map.keys()))
                    dst = random.choice(list(self.hostname_ip_map.keys()))

                    if src not in data: 
                        data[src] = dict()
                    if 'Processes' not in data[src]: 
                        data[src]['Processes'] = []
                    
                    # Create a 4444 connection between them 
                    fp = generate_fp(
                        self.hostname_ip_map[src], 
                        self.hostname_ip_map[dst]
                    )
                    data[src]['Processes'] += fp 
                    data[src]['System info'] = true_observation[src]['System info']


        # (CybORG FIX) Execute PrivEsc if Username == root/SYSTEM
        # for agent_name, agent_object in self.agent_interfaces.items():
        #     if agent_name == 'Red':
        #         privilege_flag = False

        #         if isinstance(self.action[agent_name], ExploitAction):# or isinstance(self.action[agent_name], ExploitRemoteService):
        #             exploit_action_ip = self.action[agent_name].ip_address
        #             exploit_action_port = self.action[agent_name].exploit_port
        #             # exploit_action_name = self.action[agent_name].exploit_name

        #             for host, info in true_observation.items():
        #                 if host != 'success':
        #                     if info['Interface'][0]['IP Address'] == exploit_action_ip:
        #                         for proc in info['Processes']:
        #                             if 'Connections' in proc.keys():
        #                                 for connection in proc['Connections']:
        #                                     if connection['local_port'] == exploit_action_port:
        #                                         if proc['Username'] == 'root' or proc['Username'] == 'SYSTEM':
        #                                             temp_host = host
        #                                             privilege_flag = True
        #                                             break

        #         if privilege_flag:
        #             dummy_priv = PrivilegeEscalate(session=0, agent=agent_name, hostname=temp_host)
        #             priv_results = self.step(agent=agent_name, action=dummy_priv)
        #             # priv_results = self.step_red(agent=agent_name, action=dummy_priv)
        #             # return priv_results
    ...
    def _create_agents(self, agent_classes: dict = None) -> dict:
        agents = {}
        desired_order = ["Blue", "Red", "Green"]
        scenario_agents = self.scenario.agents

        for agent_name in desired_order:
            if agent_name in scenario_agents:
        ...
        for agent_name in scenario_agents:
            if agent_name not in desired_order:
                agent_info = self.scenario.get_agent_info(agent_name)
                if agent_classes is not None and agent_name in agent_classes:
                    agent_class = agent_classes[agent_name]
                else:
    ...
    def _filter_obs(self, obs: Observation, agent_name=None):
        ...
        ips = list(self.hostname_ip_map.values())
        whitelist = []
        for h in obs.data:
            if h != 'success':
                if 'Processes' in obs.data[h]:
                    for conn in obs.data[h]['Processes']:
                        if 'Connections' in conn.keys():
                            for conn_items in conn['Connections']:
                                if 'remote_address' in conn_items:
                                    if str(conn_items['remote_address']) not in whitelist:
                                        whitelist.append(conn_items['remote_address'])
        ips.extend(whitelist)
        obs.filter_addresses(ips=ips, cidrs=subnets, include_localhost=False)
    ...
    def close_docker(self, agent: str):
        for agent_name, agent_object in self.agent_interfaces.items():
            if agent_name == agent:
                agent_object.close_container()
