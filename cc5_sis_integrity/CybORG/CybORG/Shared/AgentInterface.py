...
from CybORG.Shared.RewardCalculator import RewardCalculator, EmptyRewardCalculator, SuccessRewardCalculator
...
class AgentInterface:
    ...
    def set_init_obs(self, init_obs, true_obs):
        # print("set_initi_obs in AgentInterface")
    ...
    def get_action(self, observation: Observation, action_space: dict = None, true_obs: dict = None):
        """Gets an action from the agent to perform on the environment"""
        # print("AgentInterface get action")
        # action_mapping_table = {"my_value": "Diana"}
        if isinstance(observation, Observation):
            observation = observation.data
        if action_space is None:
            action_space = self.action_space.get_action_space()
        self.last_action = self.agent.get_action(observation, action_space, true_obs)
    ...
    def reset(self, initial_host=None):
        self.hostname = {}
        self.username = {}
        self.group_name = {}
        self.process_name = {}
        ...
        self.agent.end_episode()
        if self.agent.__class__.__name__ in ['BlueReactRemoveAgent', 'BlueReactRestoreAgent', 'BlueReactRestoreContentAgent']:
            self.agent.set_initial_host(initial_host)
    ...
    def create_reward_calculator(self, reward_calculator: str, agent_name: str, scenario: Scenario) -> RewardCalculator:
        ...
        elif reward_calculator == 'Success':
            calc = SuccessRewardCalculator(agent_name)
