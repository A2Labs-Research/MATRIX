        ...
        for child in session.children.values():
            for artifact in session.artifacts:
                if len(state.hosts[child.host].events[artifact]) > 0:
                    ...
                    if artifact == 'DefaceFront' or artifact == 'DefaceAuth':
                        obs.add_defaced_activity(hostid=child.host, defaced_status=True, 
                                                 host_ip=state.hosts[child.host].interfaces[1].ip_address, 
                                                 green_complainers=state.hosts[child.host].green_complainers)
                if artifact != 'DefaceFront' and artifact != 'DefaceAuth':
