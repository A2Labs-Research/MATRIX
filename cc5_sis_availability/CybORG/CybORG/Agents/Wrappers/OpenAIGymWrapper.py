...
class OpenAIGymWrapper(Env, BaseWrapper):
    def __init__(self, agent_name: str, env: BaseWrapper = None, agent: BaseAgent = None, paddings=False):
        super().__init__(env, agent_name, paddings)
    ...
    def step(self, action: Union[int, List[int]] = None) -> (object, float, bool, dict):
        self.action = action
        flag = self.process_action(action)
        result = self.env.step(self.agent_name, action, flag)
    ...
    # Returns a flag or a list of flags that show which of the actions are chosen to be negative
    def process_action(self,action: Union[int, List[int]]) -> Union[bool, List[bool]]:
        if isinstance(action, int) or isinstance(action, np.int64) or isinstance(action, np.int32) or isinstance(action, np.ndarray) or action.shape == ():
            flag = True
            if action < 0:
                flag = False
        elif isinstance(action, list):
            flag = [item >= 0 for item in action]
        else:
            raise TypeError("action must be an integer or a list of integers")
        return flag
