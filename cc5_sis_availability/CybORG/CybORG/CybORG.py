...
class CybORG(CybORGLogger):
    ...
    def _create_env_controller(self, env_config, agents) -> EnvironmentController:
        ...
        """
        if self.env == "sim":
            return SimulationController(self.scenario_file, agents=agents, random_red_init=self.random_red_init, random_topologies=self.random_topologies, seed=self.seed, hp=self.hp)
        if self.env == "aws":
            pass
            # if env_config:
